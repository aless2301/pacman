...
def musica_inicio():
    pygame.mixer.init()
    try:
        pygame.mixer.music.load("Pacman-sound.mp3")
...
def musica_pacman():
    pygame.mixer.init()
    try:
        pygame.mixer.music.load("PacmanWakaWaka04.wav")
...
def reproducir_muerte():
    ...
    pygame.mixer.init()
    try:
        sonido_muerte = pygame.mixer.Sound("death.mp3")
        sonido_muerte.play()
    except:
        print("No se encontró el archivo de sonido")
...
def display():
    ...
    if state == "PLAYING" and not game_over:
        ...
        if en_interseccion and not g0.en_interseccion:
            d1, d2 = a_star(g0, g1, pacman_pos)
            g0.dir = d1
            g1.dir = d2
    
            g0.en_interseccion = True
            g1.en_interseccion = True

        if not en_interseccion:
            g0.en_interseccion = False
            g1.en_interseccion = False

        
        for g in (ghosts):
            g.draw()
            
            if g == ghosts[0] or g == ghosts[1]: 
